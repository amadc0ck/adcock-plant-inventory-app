...
Scroll is preserved the same way (v1.52.2), for the page, the modal and the Gallery's horizontal `.gal-scroll` rows — but **only when the render is the same view as the last one**. Restoring it unconditionally would be wrong: navigating to a new screen should start at the top. Without this, toggling one checkbox in a long list threw the user back to the top of it.

**There are two keys, and the difference matters (v2.17.0, PHOTO-5).** `pageViewKey()` is tab + record id + `reportView` and **deliberately excludes the modal**; `currentViewKey()` is that plus the modal type. Page scroll restores whenever the page key is unchanged, modal scroll only when the full key is — so a modal opening or closing leaves the page where it was, while a freshly opened modal still starts at its own top. Originally the modal type was in the single key, which made opening the attach picker count as navigation and closing it count as navigation again: filing photos from the Gallery threw you to the top twice per photo. **A modal is an overlay on the page behind it, not a different screen** — anything keyed on "same view" has to decide which of the two it means. `debouncedRender()` (150ms) batches rapid updates — necessary once Inbox counts reached the hundreds, since each async thumbnail load used to trigger its own full rebuild.
...
- **A photo feed is a sequence, so it must not use CSS columns (v2.18.0).** `.photo-feed` was `columns:3`, which flows content down column one and back to the top for column two — chronological photos then run out of order and back again while scrolling straight down, and uncropped frames made every card a different height. It now mirrors `.card-grid` (1/2/3 columns by breakpoint, `aspect-ratio:1`, `object-fit:cover`), so a location's photos and the Gallery tab agree. Leave `object-position` at `top` in the stylesheet: `photoImg()` sets the focal point inline and a fixed value here overrides it on every thumbnail. Cropping loses nothing — `.lightbox-body img` is `object-fit:contain`, so the whole frame is one tap away.
...
- **Never revoke a blob URL without blanking the `<img>` that holds it (v2.17.1).** A revoked `blob:` URL in a live `src` renders as a broken-image icon and `net::ERR_FILE_NOT_FOUND` in the console — it looks exactly like a missing file or a Drive failure, and sent the first diagnosis to the wrong system entirely. `evictPhotoCache(protectId)` takes the id that has just arrived and refuses to evict it, because `ensurePhotoLoaded()` caches and queues a photo before its `<img>` gets a src, so `idsOnScreen()` cannot see it; past ~600 rendered tiles every older entry is live and the newcomer is the only evictable one. `releaseRenderedPhoto()` blanks and re-observes anything whose blob really is dropped. Note `idsOnScreen()` means **in the DOM**, not in the viewport — so above `PHOTO_CACHE_LIMIT` rendered tiles the cache grows rather than shrinking, which is the deliberate lesser evil.
- **A failed photo fetch must hand the element back to the observer (v2.17.1).** `observeLazyPhotos()` calls `obs.unobserve()` *before* `ensurePhotoLoaded()`, so nothing is watching the element while the request is in flight. If the request fails and the catch only clears the in-flight flag, that tile is blank for the life of the page — no observer, no retry, no way to trigger one.
...
- **Pickers are shared components, not per-modal lists (v1.72–1.73).** `plantPicker()` and `locationPicker()` are each defined once and used by every modal that needs one; both substitute `{id}` into a caller-supplied snippet. Inside a form with a Save button they must **stage** rather than act — `setEditPhotoPlant()` / `setBatchPlant()` — or picking commits a change the user has not confirmed. `locationPicker()` also writes `noteRecentLocation()`, which is what feeds the Inbox jump list; a new picker that skips it silently degrades that list. `plantPicker()` gained the same thing in v2.17.0 (PHOTO-6), and **it shipped broken** — only two of the seven commit paths called `notePlantUse()`, and not the one the Gallery uses, so the list was permanently empty and rendered nothing. Fixed in v2.17.2. All seven now record: `assignPlantToPhoto`, `makePrimaryPlantForPhoto`, `bulkAttachToPlant`, `applyGalleryBatch`, `assignPhotosToPlant`, `attachInboxPhotoToPlant`, and the edit-photo form in `wireModalForms()`; `unassignPlantFromPhoto` deliberately does not. It is **not** called from `plantPicker` itself, tempting as that is, because `setBatchPlant()` and `setEditPhotoPlant()` only stage a pick into a form — recording there would list plants that were chosen and then abandoned. **A new path that attaches a photo to a plant must call it**, or the list silently stays empty. The recents row ignores the picker's search and filters and hides itself while either is active.
- **`html`/`body` must use `overflow-x:clip`, never `hidden` (v2.17.3).** `hidden` makes the element a scroll container, and `position:sticky` inside a scroll container stops pinning to the viewport — so it disables **every** sticky header in the app at once, with no error and nothing linking cause to effect. v2.6.1 added `overflow-x:hidden` to stop screens scrolling sideways and killed the sticky headers from v1.73.2 in the same stroke; it went unnoticed for months. `clip` clips without creating a scroll container, so it stops the sideways scroll *and* leaves sticky working. If sideways scrolling ever returns, fix the element that overflows — do not reinstate `hidden`.
